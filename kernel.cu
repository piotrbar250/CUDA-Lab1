...
__global__ void addKernel(int* a, unsigned int size)
{
    int i = threadIdx.x;
    unsigned int sizehalf = (size + 1) / 2;
    if (i + sizehalf < size)
    {
        //printf("i, a[i], a[i + sizehalf], suma, %d, %d, %d\n", i, a[i], a[i + sizehalf], a[i] + a[i + sizehalf]);
        a[i] += a[i + sizehalf];
    }
}
...
cudaError_t addWithCuda(int* a, unsigned int size)
{
    ...
    int blocksCnt = size / THREADCOUNT;
    ...
    while (size > 1)
    {
        ...
        addKernel << <1, newSize >> > (dev_a, size);
        size = newSize;
    }
